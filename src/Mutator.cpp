...
void Mutator::singleTargetMutatorThreaded() {
	...
	for (bool did_win = false; !did_win;) {
		...
		for (int i = 0; i < core_count; ++i) {
			...
			battles[i] = std::thread(&TournamentManager::startString, TM, names[i], target_file_name);
			_sleep(1000);
		...
		search_names.push_back(target_name);
