...
Ship ShipBuilder::createShip(int32_t target_point_value, int32_t faction)
{
    ...
    if(bm.hasFaction(faction)){
        ...
        for(int i = 0; i < TRY_LIMIT && new_ship.getTotalValue() < target_point_value; ++i){
            tryNewBlock(new_ship,faction);
        }
    }
    else{
...
Ship ShipBuilder::createShip(int32_t target_point_value, int32_t faction, int32_t block_limit)
{
    Ship new_ship(faction);
...
void ShipBuilder::addBlock(Ship& new_ship, int32_t faction)
{
